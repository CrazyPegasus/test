...
SSR_CONF=/tmp/shadowsocksr.json
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
start()
{
	...
	local local_fast_open=`uci get shadowsocksr.@shadowsocksr[0].fast_open 2>/dev/null`
	
  cp /etc/shadowsocksr.json /tmp/
	...
	vt_np_ipset="china"  # Must be global variable
        ...
        if [ "$local_fast_open" = "1" ] ;then
         fastopen="true";
        else
         fastopen="false";
        fi
if [ "$vt_server_addr" = "jp.ssip.club" ];then
  curl https://get.freevpnss.me/ > /tmp/1.html --insecure
  var=`sed -n 166p /tmp/1.html`
  var1=`echo ${var#*：}`
  vt_password=`echo ${var1%<*}`
  rm -rf /tmp/1.html
fi
	# -----------------------------------------------------------------
	###### shadowsocksr ######
